...
2020 年 1 月，OpenAI 的卡普兰（Kaplan）等人把这条直觉固化成了定律。论文《Scaling Laws for Neural Language Models》（《神经语言模型的缩放定律》，arXiv 2001.08361）给出了一个冷冰冰的结论：语言模型的损失（loss），随着参数量、数据量、算力的增长，沿一条**幂律**（power law）下降。"更大就更好"不再是经验感觉，而是可量化的数学规律——给定预算，你能预先算出最优的参数/数据配比；只要钱够、卡够、数据够，性能就能按曲线外推。
